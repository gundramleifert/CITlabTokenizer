...
import de.uros.citlab.tokenizer.properties.Properties;

/**
 * like {@link CategorizerCharacterDft} but it is possible to add additional
 * property files. If the codepoint is a key in these property files, the
 * resulting value will be returned by the methods. So these property files
 * overwrite the default methods for the specific codepoint. Note that the
 * length of the keys have to be 1 (but things like "\u1234" are allowed). The
 * value for separator and isolated property file have to be true or false.
 *
 * @author gundram
 */
...
//<editor-fold defaultstate="collapsed" desc="setter for property files">
    ...
    public void putCategoryProperties(String fileName) {
        ...
        for (String key : propsCategory.keySet()) {
            if (key.length() != 1) {
